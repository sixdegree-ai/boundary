...
from .analysis import MODE_LABELS
...
def generate_charts(df: pd.DataFrame, output_dir: Path | None = None) -> None:
...
def _hero_degradation(df: pd.DataFrame, output_dir: Path) -> None:
    ...
    # Zone bands
    fig.add_hrect(y0=95, y1=105, fillcolor=_GREEN, opacity=0.06, line_width=0,
    ...
    fig.add_hrect(y0=0, y1=80, fillcolor=_RED, opacity=0.04, line_width=0,
    ...
    if len(providers) > 0 and len(acc) > 1:
        ...
        fig.add_annotation(
            x=worst_row["num_tools"], y=worst_row["pct"] - 2,
            text=f"<b>{worst_row['pct']:.0f}%</b> with {int(worst_row['num_tools'])} tools",
            showarrow=True, arrowhead=2, arrowsize=1, arrowwidth=1.5, arrowcolor=_RED,
            ax=0, ay=40,
            font=dict(size=12, color=_RED, family=_FONT),
            bordercolor=_RED, borderwidth=1, borderpad=6,
    ...
    fig.update_layout(
        title=dict(
            text=(
                ...
                f"<br><span style='font-size:13px;color:{_TEXT_MUTED}'>Tool-calling accuracy across {len(providers)} model{'s' if len(providers) != 1 else ''} "
                f"with {int(acc['num_tools'].min())} to {int(acc['num_tools'].max())} tools available</span>"
            ),
...
def _hero_disclosure(df: pd.DataFrame, output_dir: Path) -> None:
    ...
    agg = df.groupby(["provider", "mode"]).agg(
    ...
    for r, c_idx in [(1, 1), (1, 2), (2, 1), (2, 2)]:
        fig.update_xaxes(row=r, col=c_idx, showgrid=False, tickfont=dict(size=10, color=_TEXT_MUTED),
                         linecolor=_BORDER, linewidth=1)
        fig.update_yaxes(row=r, col=c_idx, showgrid=True, gridcolor=_GRID, gridwidth=1,
                         zeroline=False, tickfont=dict(size=10, color=_TEXT_MUTED))
    ...
    if "all" in modes_present and "disclosed" in modes_present:
        ...
        if parts:
            subtitle = f"<br><span style='font-size:14px;color:{_TEXT_MUTED}'>Late disclosure: {' &nbsp;/&nbsp; '.join(parts)}</span>"
